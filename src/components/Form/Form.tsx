...
const Form: FC = () => {
    ...
    const {sortBy,  startIndex, maxResult} = useAppSelector(state => state.form)
    const [searchValue, setSearchValue] = useState('')
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        dispatch(setStartIndex(0))
        dispatch(setBooksClear())
        dispatch(fetchBooks(({search: searchValue, sortBy, startIndex, maxResult})))
        setSearchValue('')
    ...
    return (
        <form onSubmit={handleSubmit} className={styles.form}>
            <MyInput placeholder={'Enter book tittle'} value={searchValue}
