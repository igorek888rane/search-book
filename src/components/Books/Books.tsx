...
const Books: FC = () => {
    ...
    const {startIndex, maxResult, sortBy, search, filter} = useAppSelector(state => state.form)
    ...
    return (
        <div className={styles.books}>
            ...
            <div className={styles.books__list}>
                {filterBook.map(book => <BookCard key={book.id} book={book}/>)}
                {loading && <Loader/>}
            </div>
            {filterBook.length && <div className={styles.book__btn}>
                <MyButton type={'button'} onClick={loadMore}>Load More</MyButton>
            </div>}
        </div>
    );
};
